    ...
    <div onClick={onClose} style={ovl}>
      <div onClick={e => e.stopPropagation()} style={panel}>
        <div style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid #d4dae3' }}>
          <span style={{ fontFamily: 'Roboto Mono', fontSize: 11, letterSpacing: 2, color: '#2563eb' }}>MULTI-ADD STORAGE</span>
          ...
          {done != null ? (
            ...
            <>
              <div style={{ fontSize: 11, color: '#4a5568' }}>
                Selected <b>{nodeIds.length}</b> node(s){nodeIds.length !== fresh.length && <span style={{ color: '#94a3b4' }}> · {nodeIds.length - fresh.length} already have storage (skipped)</span>}.
                Will create <b style={{ color: '#2563eb' }}>{fresh.length}</b>.
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, maxHeight: 90, overflowY: 'auto' }}>
                {fresh.map(id => (
                  <span key={id} style={{ fontSize: 9, fontFamily: 'Roboto Mono', padding: '1px 5px', borderRadius: 2, color: '#2563eb', border: '1px solid rgba(37,99,235,0.3)' }}>{prefix}{id}</span>
                ))}
                {!fresh.length && <span style={{ fontSize: 10, color: '#94a3b4' }}>No free nodes in the selection.</span>}
              </div>
