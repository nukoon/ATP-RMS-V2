    ...
    <div onClick={onClose} style={ovl}>
      <div onClick={e => e.stopPropagation()} style={panel}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: '1px solid #d4dae3' }}>
          <span style={{ fontFamily: 'Roboto Mono', fontSize: 11, letterSpacing: 2, color: '#2563eb' }}>📊 FLEET DASHBOARD</span>
        ...
        <div style={{ overflowY: 'auto', padding: 14, display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 10 }}>
            ...
            <Kpi label="Utilization" value={`${Math.round(metrics.utilization * 100)}`} unit="%" color="#2563eb" />
            ...
            <Kpi label="Total Distance" value={metrics.totalDistance.toFixed(1)} unit="m" color="#4a5568" />
          ...
          <div>
            ...
            <div style={{ display: 'flex', height: 14, borderRadius: 3, overflow: 'hidden', border: '1px solid #d4dae3' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 6 }}>
              {(Object.keys(MISSION_COLOR) as Mission['status'][]).map(s => (
                <span key={s} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#4a5568' }}>
              ...
              <thead>
                <tr style={{ textAlign: 'left', color: '#64748b' }}>
                  {['AGV', 'Status', 'Battery', 'Distance', 'Active', 'Done'].map(h => (
                    <th key={h} style={{ padding: '5px 8px', borderBottom: '1px solid #d4dae3' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {perAgv.map(a => (
                  <tr key={a.id} style={{ borderBottom: '1px solid rgba(212,218,227,0.6)' }}>
                    <td style={{ padding: '5px 8px', color: '#1a2230', fontWeight: 600 }}>{a.id}</td>
                    <td style={{ padding: '5px 8px', color: STATUS_COLOR[a.status] ?? '#64748b' }}>{a.status}</td>
                    <td style={{ padding: '5px 8px', color: a.battery > 30 ? '#16a34a' : '#dc2626' }}>{a.battery}%</td>
                    <td style={{ padding: '5px 8px', color: '#4a5568' }}>{a.distance.toFixed(1)} m</td>
                    <td style={{ padding: '5px 8px', color: '#2563eb' }}>{a.active}</td>
                    <td style={{ padding: '5px 8px', color: '#15803d' }}>{a.done}</td>
                  </tr>
                ))}
                {!perAgv.length && (
                  <tr><td colSpan={6} style={{ padding: 14, color: '#94a3b4', textAlign: 'center' }}>No robots — START SIM or CONNECT</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function Kpi({ label, value, unit, color }: { label: string; value: string; unit?: string; color: string }) {
  return (
    <div style={{ background: '#f3f6fa', border: '1px solid #d4dae3', borderRadius: 4, padding: '10px 12px' }}>
      <div style={{ fontSize: 9, letterSpacing: 1, color: '#64748b', textTransform: 'uppercase', marginBottom: 5 }}>{label}</div>
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
        <span style={{ fontFamily: 'Roboto Mono', fontSize: 22, fontWeight: 700, color }}>{value}</span>
        {unit && <span style={{ fontSize: 10, color: '#94a3b4' }}>{unit}</span>}
      </div>
    </div>
  )
}
...
const SectionTitle = ({ children }: { children: React.ReactNode }) => (
  <div style={{ fontSize: 9, letterSpacing: 2, color: '#64748b', textTransform: 'uppercase', marginBottom: 8 }}>{children}</div>
)
...
const panel: React.CSSProperties = { width: 'min(820px, 92vw)', maxHeight: '88vh', background: '#ffffff', border: '1px solid #d4dae3', borderRadius: 4, display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: 'Inter, "Noto Sans JP", sans-serif', color: '#1a2230' }
