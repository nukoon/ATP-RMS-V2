...
export function Map3DCanvas({ map, robots, config, selectedRobotId, onRobotClick }: Props) {
  ...
  useEffect(() => {
    ...
    if (config.showEdges) {
      ...
      scene.add(new THREE.LineSegments(g, new THREE.LineBasicMaterial({ color: 0xb9c4d6, transparent: true, opacity: 0.45 })))
    }
    ...
    const CARGO_SIZE = 1.15   // goods footprint (m) when carried on a robot
    ...
    const buildRouteTube = (r: Robot): THREE.BufferGeometry | null => {
      ...
      return new THREE.TubeGeometry(new THREE.CatmullRomCurve3(clean), Math.max(8, clean.length * 2), 0.14, 6, false)
    }
    ...
    const syncRobots = () => {
      ...
      for (const r of list) {
        ...
        if (r.carrying && !cargo && !g.userData.cargoLoading) {
          ...
          loadGlbTemplate(GOODS_FILE).then(tpl => {
            ...
            inst.scale.setScalar(CARGO_SIZE); inst.position.set(len * 0.05, 0.7, 0)
            cg.add(inst)
          }).catch(() => {})
        }
        if (cargo) { cargo.visible = !!r.carrying; cargo.scale.setScalar(CARGO_SIZE); cargo.position.y = 0.7 }
